...
corpus <- read_csv("canons/stoker.csv")
```
...
```{r}
full_join_df_stats <- full_join_df %>% group_by(words, gender) %>% summarise(count = n()) %>% arrange(desc(count))%>%
  na.omit()

ggplot(data = full_join_df_stats) + 
  geom_bar(mapping = aes(x = words, y=count, fill = gender), stat = "identity") +
...
ner_total_sentiment <- entities_matches_sentiment %>%
  group_by(author, title, words, kind, gender) %>%
  summarise(total = mean(sentiment))  
#write_csv(ner_total_sentiment, "lovecraft_ner_sentiment.csv")
#write_csv(ner_total_sentiment, "stevenson_ner_sentiment.csv")
#write_csv(ner_total_sentiment, "stoker_ner_sentiment.csv")
#write_csv(ner_total_sentiment, "shelley_ner_sentiment.csv")

#ner_total_sentiment <- read_csv("shelley_ner_sentiment.csv")
```
...
ner_total_sentiment %>%
  ...
  ggplot(aes(words, y = total, fill = gender)) +
  geom_col() +
  scale_x_discrete(guide = guide_axis(check.overlap = TRUE)) +
  theme_economist() + scale_fill_economist()
```
...
radar_facet <- entities_matches_sentiment %>%
  select(-positive, -negative, -sentiment, -title) %>% #drop out the unnecessary columns
  filter(kind == "person") %>%
  group_by(words, kind, gender) %>%
...
radar_facet2 <- radar_facet[c("words","gender","anger", "anticipation","sadness", "fear","trust","disgust","joy","surprise")]

ggRadar(
  data = radar_facet2,
  mapping = aes(color=gender, facet = words),
  rescale = FALSE,
  interactive = TRUE,
  use.label = TRUE,
  size = 2,
...
ggradar(df)
#radar_facet2 <- radar_facet %>% select(words)
radar_facet2 <- radar_facet[c("words","anger", "anticipation","sadness", "fear","trust","disgust","joy","surprise")]
ggradar(radar_facet2, grid.max=37, grid.min=0)
```
```{r radar_plot_total, message=FALSE}
radar_facet3 <- entities_matches_sentiment %>%
  select(-positive, -negative, -sentiment, -title) %>% #drop out the unnecessary columns
  filter(kind == "person") %>%
  group_by(words, kind, gender) %>%
  summarise(across(anger:trust, sum)) %>%
  mutate(total = rowSums(across(where(is.numeric))))  %>%
  arrange(desc(total)) %>%
  filter(words == "Caroline"| words == "Diana"| words == "Cornelia"| words == "Elizabeth") #Change number to include more or fewer entities

radar_facet3 <- radar_facet3[c("words","anger", "anticipation","sadness", "fear","trust","disgust","joy","surprise")]
ggradar(radar_facet3, grid.max=350, grid.min=0)
```

```{r radar_plot_total, message=FALSE}
radar_facet4 <- entities_matches_sentiment %>%
  select(-positive, -negative, -sentiment, -title) %>% #drop out the unnecessary columns
  filter(kind == "person") %>%
  group_by(words, kind, gender) %>%
  summarise(across(anger:trust, sum)) %>%
  mutate(total = rowSums(across(where(is.numeric))))  %>%
  arrange(desc(total)) %>%
  filter(words == "Adam"| words == "John Santerre"| words == "Victor" | words == "Mr Fitzhenry") #Change number to include more or fewer entities

radar_facet4 <- radar_facet4[c("words","anger", "anticipation","sadness", "fear","trust","disgust","joy","surprise")]
ggradar(radar_facet4, grid.max=110, grid.min=0)
```
...
tidy_test_male_pronouns_he <- tidy_test %>% 
                            filter(grepl("he ", sentence))
tidy_test_male_pronouns_him <- tidy_test %>% 
                            filter(grepl("him ", sentence))
tidy_test_male_pronouns_his <- tidy_test %>% 
                            filter(grepl("his ", sentence))

tidy_test_female_pronouns_she <- tidy_test %>% 
                            filter(grepl("she ", sentence))
tidy_test_female_pronouns_her <- tidy_test %>% 
                            filter(grepl("her ", sentence))
tidy_test_female_pronouns_hers <- tidy_test %>% 
                            filter(grepl("hers", sentence))
```

```{r}
tidy_test_female_pronouns_words_she <- tidy_test_female_pronouns_she %>% ungroup() %>% unnest_tokens(word, sentence)
tidy_test_female_pronouns_words_her <- tidy_test_female_pronouns_her %>% ungroup() %>% unnest_tokens(word, sentence)
tidy_test_female_pronouns_words_hers <- tidy_test_female_pronouns_hers %>% ungroup() %>% unnest_tokens(word, sentence)

tidy_test_male_pronouns_words_he <- tidy_test_male_pronouns_he %>% ungroup() %>% unnest_tokens(word, sentence)
tidy_test_male_pronouns_words_his <- tidy_test_male_pronouns_his %>% ungroup() %>% unnest_tokens(word, sentence)
tidy_test_male_pronouns_words_him <- tidy_test_male_pronouns_him %>% ungroup() %>% unnest_tokens(word, sentence)
```
...
```{r}
#create sentiment table
test_pronouns_sentiment_female_she <- tidy_test_female_pronouns_words_she %>%
  inner_join(get_sentiments("nrc"))%>%
  distinct()%>%
  spread(sentiment, 4, fill = 0) 
test_pronouns_sentiment_female_her <- tidy_test_female_pronouns_words_her %>%
  inner_join(get_sentiments("nrc"))%>%
  distinct()%>%
  spread(sentiment, 4, fill = 0) 
test_pronouns_sentiment_female_hers <- tidy_test_female_pronouns_words_hers %>%
  inner_join(get_sentiments("nrc"))%>%
  distinct()%>%
  spread(sentiment, 4, fill = 0) 

test_pronouns_sentiment_male_he <- tidy_test_male_pronouns_words_he %>%
  inner_join(get_sentiments("nrc"))%>%
  distinct()%>%
  spread(sentiment, 4, fill = 0) 
test_pronouns_sentiment_male_his <- tidy_test_male_pronouns_words_his %>%
  inner_join(get_sentiments("nrc"))%>%
  distinct()%>%
  spread(sentiment, 4, fill = 0) 
test_pronouns_sentiment_male_him <- tidy_test_male_pronouns_words_him %>%
  inner_join(get_sentiments("nrc"))%>%
  distinct()%>%
  spread(sentiment, 4, fill = 0) 

test_pronouns_sentiment_female_she[4:13] <- lapply(test_pronouns_sentiment_female_she[4:13], function(x) as.integer(x!="0"))
test_pronouns_sentiment_female_her[4:13] <- lapply(test_pronouns_sentiment_female_her[4:13], function(x) as.integer(x!="0"))
test_pronouns_sentiment_female_hers[4:13] <- lapply(test_pronouns_sentiment_female_hers[4:13], function(x) as.integer(x!="0"))

test_pronouns_sentiment_male_he[4:13] <- lapply(test_pronouns_sentiment_male_he[4:13], function(x) as.integer(x!="0"))
test_pronouns_sentiment_male_his[4:13] <- lapply(test_pronouns_sentiment_male_his[4:13], function(x) as.integer(x!="0"))
test_pronouns_sentiment_male_him[4:13] <- lapply(test_pronouns_sentiment_male_him[4:13], function(x) as.integer(x!="0"))

```
```{r}
# computing column wise sum
test_pronouns_sentiment_female_she <- test_pronouns_sentiment_female_she[4:13]%>%
  summarise_all(sum) %>%
  mutate(Pronouns = "she",
             .before = "anger")  %>%
  select(-positive, -negative)
test_pronouns_sentiment_female_her <- test_pronouns_sentiment_female_her[4:13]%>%
  summarise_all(sum) %>%
  mutate(Pronouns = "her",
             .before = "anger")  %>%
  select(-positive, -negative)
test_pronouns_sentiment_female_hers <- test_pronouns_sentiment_female_hers[4:13]%>%
  summarise_all(sum) %>%
  mutate(Pronouns = "hers",
             .before = "anger")  %>%
  select(-positive, -negative)

test_pronouns_sentiment_female <- bind_rows(test_pronouns_sentiment_female_she,test_pronouns_sentiment_female_her, test_pronouns_sentiment_female_hers)

test_pronouns_sentiment_male_he <- test_pronouns_sentiment_male_he[4:13]%>%
  summarise_all(sum) %>%
  mutate(Pronouns = "he",
             .before = "anger")  %>%
  select(-positive, -negative)
test_pronouns_sentiment_male_him <- test_pronouns_sentiment_male_him[4:13]%>%
  summarise_all(sum) %>%
  mutate(Pronouns = "him",
             .before = "anger")  %>%
  select(-positive, -negative)
test_pronouns_sentiment_male_his <- test_pronouns_sentiment_male_his[4:13]%>%
  summarise_all(sum) %>%
  mutate(Pronouns = "his",
             .before = "anger")  %>%
  select(-positive, -negative)

test_pronouns_sentiment_male <- bind_rows(test_pronouns_sentiment_male_he,test_pronouns_sentiment_male_him, test_pronouns_sentiment_male_his)

# printing final output
#print(test_pronouns_sentiment_female)
#print(test_pronouns_sentiment_male)
```

```{r}
print(test_pronouns_sentiment_female)
ggradar(test_pronouns_sentiment_female, grid.max=5900, grid.min=0)
```

```{r}
#print(test_pronouns_sentiment_male)
ggradar(test_pronouns_sentiment_male, grid.max=5900, grid.min=0)
```
